...
const CardContent = ({ logo, text, title, subtitle, link }) => {
  return (
    <>
      <img
        src={logo}
        ...
        <div className="flex justify-between items-center">
          ...
          {link && (
            <div className="rounded-full aspect-square w-16 flex border border-solid border-black/10 p-2 shrink-0 group-hover:bg-[--color-primary-200] group-hover:text-white transition-all duration-200">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-full h-auto">
      ...
      {link && (
        <Link href={link}>
          <a className="flex items-start flex-col justify-between h-full p-[2rem] sm:p-[1.5rem] md:p-[2.4rem]">
            <CardContent logo={logo} text={text} title={title} subtitle={subtitle} link={link} />
          </a>
        </Link>
      )}
      {!link && (
        <div className="flex items-start flex-col justify-between h-full p-[2rem] sm:p-[1.5rem] md:p-[2.4rem]">
          <CardContent logo={logo} text={text} title={title} subtitle={subtitle} link={link} />
        </div>
      )}
    </div>
